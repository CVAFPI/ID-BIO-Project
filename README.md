...
4. **Parents Alert:** Using the `NTFY` APP (Available in Playstore and Apple App Store) is used to alert parents, using the built in Random topic genarator in Database Manager when clicked save it will work immediately. NOTE to privacy reasons do not share the string of numbers and letters for Security for both child and Parent!
