...
| Storage Capacity | 64 GB SSD / Storage | 2 TB SSD / Hard Drive (Recommended for multi-year logs) |
 ...
 * Standard Chipset Suppliers: Ensure your system uses standard Intel or AMD 64-bit x86 processors, or standard ARM64 (aarch64) Single Board Computers (such as Raspberry Pi 4/5 running a 64-bit OS). Avoid obscure, unbranded x86 clones lacking stable Linux kernel driver support.
 * Storage Allocation for 24/7 Logging: While basic setups run on 64 GB, deploying a 2 TB drive is strongly recommended for schools running the kiosk continuously (24/7/365) to store multi-year attendance archives (logs_YYYY-MM-DD.csv), local database backups, and system updates.
 ...
 * DO NOT expose your NTFY notification tokens: Keep secret strings completely private to protect student data and secure parent alert channels.
 ...
 * Select KDE Plasma during the Debian installation task selector, or install it post-installation using:
 ...
 * Performs system package updates (sudo apt update and upgrades).
...
Because scanner command barcodes trigger hardware actions (such as emergency shutdowns via CD=EMERSHUTDOWNSYSSU62#9CVAFPI) and background scripts require root privileges without human interaction, you must configure passwordless sudo for your kiosk user.
...
├── logger.py                         # Internal log processing utility
...
Scanning any of these reference command barcodes using a physical scanner immediately executes system-level operations:
 ...
 * GUI Method: Open System Settings → Autostart → Add... → Add Application or Script... → Select id_bio.desktop.
 * Terminal Method:
 ...
 * 100% Local Data Privacy: All attendance logs stay strictly on local school hardware in .csv format—no external cloud harvesting.
 ...
 * GitHub Issues: Open a ticket directly on our GitHub Issues Page.
